...
static int32_t node_mlen(void *km, const gfa_t *g, uint32_t v, mg128_v *mini, const void *h, int32_t n_seeds, const uint64_t *seeds, int32_t *qs, int32_t *qe)
{
	...
	*qs = *qe = -1;
	...
	for (i = 0; i < mini->n; ++i) {
		...
		if (v&1) v_pos = s->len - (v_pos + 1 - MG_SHORT_KK) - 1;
		assert(v_pos >= 0);
		for (j = 0; j < n; ++j) {
	...
	mlen = mg_anchor2mlen(km, MG_SHORT_KK, n_a, a, qs, qe);
	assert(mlen >= 0 && *qe >= *qs);
	kfree(km, a);
	return mlen;
}
...
static int32_t path_mlen(sp_node_t **out, int32_t start, khash_t(sp) *h, int32_t qlen)
{
	...
	for (i = out[start]->pre; i && out[i]->pre; i = out[i]->pre) {
		...
		if (k != kh_end(h)) {
			...
			int32_t ovlp;
			if (z->qe <= z->qs || z->qs >= qlen) continue;
			ovlp = z->qe <= qlen? z->qe - z->qs : qlen - z->qs;
			mlen += z->mlen * (double)ovlp / (z->qe - z->qs);
		}
	}
	return (int32_t)(mlen + .499);
}
	...
	while (kavl_size(head, root) > 0) {
		...
		if (k != kh_end(h2)) { // we have reached one dst vertex
			...
			for (j = 0; j < cnt; ++j) {
				...
				mlen = path_mlen(out, n_out - 1, h, t->qlen);
				...
				if (t->n_path == 0) { // keep the shortest path
					...
					if (dist == t->target_dist && t->check_hash && r->hash == t->target_hash) { // we found the target path
						...
						if (d1 - mlen/MG_SHORT_KK < d0 - t->mlen/MG_SHORT_KK) copy = 1;
					}
				}
				if (copy) {
		...
		for (i = 0; i < nv; ++i) { // visit all neighbors
			...
			if (absent) { // a new vertex visited
				q->k = 0, q->qs = q->qe = -1;
				q->mlen = d + ai->lw <= max_dist? node_mlen(km, g, ai->w, &mini, h_seeds, n_seeds, seeds, &q->qs, &q->qe) : 0;
