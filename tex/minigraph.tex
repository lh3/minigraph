...
GRCh38~\cite{Schneider:2017aa} are a mosaic of haplotypes with each derived
...
particular with short reads~\cite{Li:2018aa}. This underrepresentation of
...
GRCh38~\cite{Schneider:2017aa,Wenger_2019}. There are already more than ten
...
The other class of methods encode multiple genomes into a sequence graph,
...
genome or gene set that is shared across the majority of the strains or species
...
coordinate and doesn't allow the mapping of genomic features. It cannot serve
...
In addition to the deBGA solution, we can also derive a reference pangenome
graph from a single linear multi-sequence alignment (MSA)~\cite{Dilthey_2015}.
...
sources and then incorporate these variants, often in the VCF format, into
the reference genome as alternative
paths~\cite{Eggertsson:2017aa,Rakocevic_2019,Garrison:2018aa,Sibbesen:2018aa,Biederstedt:2018aa}.
However, because VCF doesn't define coordinates on insertions, this approach
...
graphs encode structural variations of length 100bp or longer without haplotype
...
\caption{\csentence{Example rGFA and GAF formats.} {\bf (a)} Example rGFA
  ...
  sequence. {\bf (c)} Example GAF format, using the vertex coordinate, for
...
sequence graphs. Fig.~\ref{fig:rgfa}a shows an example GFA format that encodes
Fig.~\ref{fig:rgfa}b. Each line starting with letter ``${\tt S}$'' corresponds
to a vertex and each line starting with ``${\tt L}$'' corresponds to a
bidirectional edge.

A sequence graph in the GFA format naturally defines a \emph{vertex coordinate}
system in the sense that each base in the graph is uniquely indexed by a
2-tuple $({\rm vertexID},{\rm vertexOffset})$. For example, in
Fig~\ref{fig:rgfa}a, the base at position $({\rm s2},2)$ is ``{\tt G}''.
However, this vertex coordinate is decoupled from linear annotations and is
sensitive to many graph operations. A basic sequence graph is inadequate for a
reference graph.

For a de Bruijn graph, we often attach sequences to edges instead of vertices.
To avoid confusion, in this article, we also call a vertex as a \emph{segment}
and call an edge as a \emph{link}, following the GFA terminology.
...
requirement in fact improses a strong restriction on the types of graphs rGFA
can encode: it forbids the collapse of different regions from one sequence,
which would often happen in a cDBG. We consider this restriction as an
advantage of rGFA because it requires the graph to have a ``linear'' flavor
intuitively and simplifies the data structure to store the graph. In
comparison, the vg graph~\cite{Garrison:2018aa} encodes linear sequences with
paths and allows the collapse of regions from one sequence (see Discussion). It
is more flexible and can work with cDBGs, but it is more complex and permits
collapsed graphs that are not suitable as reference graphs.

For simplicity, rGFA also disallows overlaps between edges and forbids multiple
edges (more than one edges between the same pair of vertices). These two
restrictions help to avoid ambituity and reduce the complexity in
...
the graph. This gives the approximate mapping locations. Minigraph does not
perform base-level alignment. This is because the graph we construct encodes
structural variations and rarely contains paths similar at the base level. The
best mapping is often clear without base alignment.

\begin{table}[b]
\caption{Performace of sequence-to-graph mapping}\label{tab:mgvga}
\begin{tabular}{lrr}
\hline
...
reads over 8 CPU threads. Partly because minigraph does not perform base
alignment, it is faster than GraphAligner and uses less memory at the peak.

As is shown in Table~\ref{tab:mgvga}, minigraph is more accurate than
GraphAligner. This is counterintuitive given that GraphAligner does base
...
is better at disambiguiating similar graph paths.  Combining the strength of
both could lead to a better graph mapper. We do plan to implement base-level
alignment in minigraph in future.

We have also tried vg v1.21.0. It indexd the same graph in 14.7 wall-clock
hours and mapped the simulated reads in 1.8 hours over 8 threads, tens of times
slower than minigraph and GraphAligner. However, it seems that with the default
setting vg is unable to map any noisy long reads. Vg is primarily designed for
short reads after all.

\subsection*{Generating pangenome graphs}

Fig.~\ref{fig:mg}b shows how minigraph constructs a pangenome graph (see
Methods for details). In the graph construction mode, minigraph only considers
structural variations (SVs) of 100bp--100kb in length. It ignores smaller
variants partly because the minigraph mapping algorithm does not work well with
short segments in rGFA and because smaller variants are easier to analyze with
standard methods. Minigraph ignores SVs in alignments shorter than 100kb.
For each input assembly, it filters out regions covered two or more primary
...
IGV~\cite{Robinson:2011aa} and identified a few causes. First, an INDEL longer
than 100bp called by one caller may be split into two shorter INDELs by the
other caller. Second, dipcall skips regions involving high desity of SNPs or
involving both long insertions and long deletions, but minigraph calls SVs in
such regions. Overall, we believe minigraph and dipcall call similar sets of
SVs.
...
\caption{\csentence{Characteristics of the human and the great ape graphs.} {\bf
  ...
  tandem repeat with the unit motif length $\ge$7bp. STR: short randem repeat,
...
Variations in the human graph are enriched with Alus or VNTRs
...
suggesting subtelomeres tend to harbor strucutural variations anyway. We also
...
are probably correlated events. Without base-level alignment, minigraph may
...
lineage. Two thirds of autosomal insertions in this categority are segmental
...
29.2Mb in length, intersecting 0.7\% of confident regions in the SynDip
benchmark dataset~\cite{Li:2018aa}; 0.7\% of truth SNPs are contained in
blacklist regions -- true SNPs are not enriched in blacklist regions.

We mapped short reads used in~\cite{Li:2018aa} with minimap2 and called
variants with GATK v4.1.2~\cite{Depristo:2011vn}. Compared to the This callset
contains 32,879 false positive SNPs, 21\% of which fall in blacklist regions --
false SNP calls are highly enriched in this $<$1\% of human genome. This
...
represents a stable sequence with a path through the sequence graph. A segment
...
segments to tell which {\sf s1} corresponds to; this can be more complicated
...
separate events. With the minigraph procedure, the two SVs are more likely to
be aligned together as long as they are similar and are sufficiently different
from the reference allele. To some extent, minigraph is similar to partial
order alignment~\cite{Lee_2002} for multiple sequence alignment. It is
reasonably robust to different representations of the same SV.

When we refer to a SNP, we often use its chromosomal coordiante such as
``chr1:12345''. We rarely do so for SVs because their positions are sensitive
to alignment and SV callers. The more consistent SV representation impiled by a
pangenome graph will help to alleviate the issue and subsequently facilitate
the genotyping of SVs~\cite{Hickey_2020,Eggertsson_2019,Chen_2019}.
...
referece, we may blacklist regions enriched with SVs that lead to small variant
...
perform read alignment against a graph but analyze the results in the linear
coordinate.  In our view, a pangenome reference is not to replace the linear
reference but to supplement it.
...
tools. We envision that a pangenome graph reference will become an effective
...
\paragraph*{A minimap2-like algorithm}
Similar to minimap2, minigraph uses minimizers on segments as seeds. It also
uses a similar chaining algorithm but with different scoring and with a new
heuristic to speed up chaining over long distances. For the completeness of
this article, we will describe linear chaining in full.
...
We implement 1-dimension RMQ with a modified AVL tree, a type of balanced
...
that completely ignores the graph tolopology. It then applies another round of
chaining while considering the topology.

We say linear chain $L_i$ \emph{preceeds} $L_j$, written as $L_i\prec L_j$, if
...
between $L_i$ and $L_j$, minigraph ignores base sequences and only considers
...
graph chain that leads to the best base alignment. Second, minigraph only
...
Minigraph scans primary chains in ortholognal regions and identifies subregions
...
a $\ge$100bp region with base-level identity below 80\%. In the end, minigraph
augments the existing graph with identified variations (Fig.~\ref{fig:mg}b).
