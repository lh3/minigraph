...
function mg_cmd_anno(args)
{
	...
	var fn_rmout = null, fn_etrf = null, fn_dust = null, fn_gap = null, fn_paf = null, fn_centro = null, fn_bb = null, fn_sd = null, fn_ld = null, print_bb = 0;
	while ((c = getopt(args, "e:p:g:d:r:c:l:S:L:b:s:mx:X:B")) != null) {
	...
	if (args.length - getopt.ind < 1) {
		...
		print("  -B          print segdup/longdust info to stderr");
	...
	if (fn_bb) {
		...
		while (file.readline(buf) >= 0) {
			...
			if (key in bb) {
				...
				bb[key][10] = tot_ld;
				if (print_bb)
					warn(t[0], t[1], t[2], tot_len, tot_sd, ref_len, tot_ld);
			}
		}
		file.close();
	}
	...
	for (var i = 0; i < bba.length; ++i) {
		...
		if ((m = /^(\S+)_(\d+)_(\d+)/.exec(key)) == null)
			throw("Bug!");
		var x = {}, t = [m[1], m[2], m[3]];
		if (fn_bb) t.push(bb[key][2], bb[key][3], bb[key][4], bb[key][5], bb[key][6], bb[key][7], bb[key][8], bb[key][9], bb[key][10]);
		else t.push(len);
		...
		if (fn_bb) {
			x['_totbb'] = t[8];
			if (fn_sd) x['_segdup'] = t[9];
			if (fn_ld) x['_ldust'] = t[11];
		}
		if ((type == 'partial' || type == 'self' || type == 'none' || type == 'mixed') && fn_bb) {
			if (t[8] >= 50 && t[11] >= t[8] * min_ld_frac)
				type = 'ldust';
