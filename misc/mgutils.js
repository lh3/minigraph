...
{
	var c, min_rm_div = 0.2, min_rm_sc = 300, micro_cap = 6, min_feat_len = 30, min_centro_len = 200, mobile = false, max_mobile_div = 2.0, min_segdup_frac = 0.2, min_ld_frac = 0.7;
	var fn_rmout = null, fn_etrf = null, fn_dust = null, fn_gap = null, fn_paf = null, fn_centro = null, fn_bb = null, fn_sd = null, fn_ld = null;
	while ((c = getopt(args, "e:p:g:d:r:c:l:S:L:b:s:mx:X:")) != null) {
	...
	if (args.length - getopt.ind < 1) {
		...
		print("  -S FLOAT    min segdup fraction [" + min_segdup_frac + "]");
		print("  -x FILE     longdust file (paste gfa2bed bedcov) [null]");
		print("  -X FLOAT    min longdust fraction [" + min_ld_frac + "]");
		print("  -m          annotate AluY and L1HS separately");
		print("Notes: -s and -x generated by");
		print("  paste <(gfatools g.gfa) <(bedtk cov reg.bed <(gfatools g.gfa)) | cut -f1-5,9,10");
		exit(1);
	}
	...
	if (fn_bb) {
		...
		if (fn_ld) {
			...
			while (file.readline(buf) >= 0) {
				var t = buf.toString().split("\t");
				ld[t[3]] = [parseInt(t[4]), parseInt(t[2]) - parseInt(t[1]), parseInt(t[6])];
			}
			file.close();
		}
		...
		while (file.readline(buf) >= 0) {
			...
			if (key in bb) {
				...
				var s = t[11].split(","), tot_len = 0, tot_sd = 0, tot_ld = 0, ref_len = 0;
				var dup = {};
				for (var i = 1; i < s.length - 1; ++i) {
					if (seg[s[i]] == null && ld[s[i]] == null) continue;
					if (dup[s[i]]) continue;
					dup[s[i]] = 1;
					if (seg[s[i]] != null) {
						...
						if (ld[s[i]] != null)
							tot_ld += ld[s[i]][2];
					} else if (ld[s[i]] != null) { // no need to consider seg[s[i]] as it is done above
						tot_len += ld[s[i]][1], tot_ld += ld[s[i]][2];
						if (ld[s[i]][0] == 0)
							ref_len += ld[s[i]][1];
					}
				}
	...
	for (var i = 0; i < bba.length; ++i) {
		...
		if ((type == 'partial' || type == 'self' || type == 'none' || type == 'mixed') && fn_bb) {
			if (t[8] >= 50 && t[10] >= t[8] * min_ld_frac)
				type = 'ldust';
			if (t[8] >= 1000 && t[9] >= t[8] * min_segdup_frac)
				type = 'segdup';
		}
		t.push(type);
		for (var c in x)
...
function mg_cmd_merge(args)
{
	...
	if (fn_anno) {
		...
		while (file.readline(buf) >= 0) {
			...
			anno[key] = t[11];
			var totbb = parseInt(t[8]), segdup = parseInt(t[9]), ldust = parseInt(t[10]);
			if (totbb > 0)
				anno_len[key] = [totbb, segdup, ldust];
		}
		file.close();
	}
	...
	print('##INFO=<ID=LBUBBLE,Number=1,Type=Integer,Description="Total sequence length in the bubble">');
	print('##INFO=<ID=LSEGDUP,Number=1,Type=Integer,Description="Length of segdup">');
	print('##INFO=<ID=LDUST,Number=1,Type=Integer,Description="Length of low-complex regions">');
	...
	while (file.readline(buf) >= 0) {
		...
		if (anno_len[key] != null) {
			info.push("LBUBBLE=" + anno_len[key][0]);
			info.push("LSEGDUP=" + anno_len[key][1]);
			info.push("LDUST=" + anno_len[key][2]);
		}
